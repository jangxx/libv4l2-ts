...
async function main() {
	...
	for (let i = 0; i < req.count; i++) {
		...
		buffers.push(v4l2_mmap(buf.length, PROT_READ | PROT_WRITE, MAP_SHARED, fd, buf.m.offset));
	}
	...
	for (let i = 0; i < 20; i++) {
		...
		console.log(
			`res: ${fmt.fmt.pix.width}x${fmt.fmt.pix.height} fmt: ${fmt.fmt.pix.pixelformat} bytes: ${buf.bytesused} buffer_index: ${buf.index}`,
		);
